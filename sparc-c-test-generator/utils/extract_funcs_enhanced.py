...
def create_predefined_functions(folder_results: Dict[str, Dict], output_file: str) -> None:
    """
    Create a predefined functions from folder extraction results.
    
    Args:
        folder_results: Results from extract_all_functions_from_folder
        output_file: Path to save the predefined functions
    """
    ...
    for file_path, result in folder_results.items():
        num_functions = len(result["functions"])
        predefined_functions["summary"]["total_functions"] += num_functions
        
        if num_functions > 0:
            predefined_functions["summary"]["files_with_functions"] += 1
        
        predefined_functions["files"][file_path] = {
            "function_count": num_functions,
            "functions": [f["name"] for f in result["functions"]],
            "headers_processed": result.get("headers_processed", []),
            "implementation_files": result.get("implementation_files", [])
        }
        
        # Add all functions to combined list with source file info
        for func in result["functions"]:
            func_with_source = func.copy()
            func_with_source["source_file_path"] = file_path
            predefined_functions["all_functions"].append(func_with_source)
    
    with open(output_file, 'w', encoding='utf-8') as f:
        json.dump(predefined_functions, f, indent=2, ensure_ascii=False)
    
    print(f"[+] predefined functions saved to {output_file}")
    print(f"[+] Summary: {predefined_functions['summary']['total_functions']} functions from {predefined_functions['summary']['files_with_functions']}/{predefined_functions['summary']['total_files']} files")
