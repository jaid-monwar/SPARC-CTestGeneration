...
# Unit Test Generation and Execution Script for Algorithm Subjects
# This script generates and runs unit tests for algorithm-based subjects
# (C2Rust translated algorithms from TheAlgorithms/C)
...
if [ $# -eq 0 ]; then
    ...
    echo "Examples:"
    echo "  $0 ds_avl_tree"
    echo "  $0 ds_avl_tree test/projects"
    echo "  $0 ds_avl_tree test/projects all"
    echo "  $0 ds_avl_tree test/projects all false multiagent true"
    echo "  $0 ml_k_means_clustering test/projects all false monolithic true"
    echo "  $0 hash_blake2b test/projects all false monolithic true gemini"
    echo "  $0 sorting_multikey_quick_sort test/projects all false monolithic true gpt 1"
    echo ""
    echo "Supported algorithm subjects:"
    echo ""
    echo "  Audio:"
    echo "    audio_alaw               - A-law audio compression"
    echo ""
    echo "  Cipher:"
    echo "    cipher_affine            - Affine cipher encryption"
    echo ""
    echo "  Conversions:"
    echo "    conv_decimal_to_any_base - Decimal to any base conversion"
    echo "    conv_infix_to_postfix    - Infix to postfix expression conversion"
    echo "    conv_infix_to_postfix2   - Infix to postfix (variant 2)"
    echo ""
    echo "  Dynamic Programming:"
    echo "    dp_lcs                   - Longest Common Subsequence"
    echo ""
    echo "  Data Structures:"
    echo "    ds_ascending_priority_queue - Ascending priority queue"
    echo "    ds_avl_tree              - AVL Tree implementation"
    echo "    ds_binary_search_tree    - Binary Search Tree"
    echo "    ds_circular_doubly_linked_list - Circular doubly linked list"
    echo "    ds_doubly_linked_list    - Doubly linked list"
    echo "    ds_dynamic_stack         - Dynamic stack"
    echo "    ds_max_heap              - Max heap"
    echo "    ds_min_heap              - Min heap"
    echo "    ds_red_black_tree        - Red-Black Tree implementation"
    echo "    ds_segment_tree          - Segment tree"
    echo "    ds_singly_link_list_deletion - Singly linked list deletion"
    echo "    ds_threaded_binary_trees - Threaded binary trees"
    echo "    ds_trie                  - Trie data structure"
    echo "    ds_vector                - Dynamic vector"
    echo "    ds_words_alphabetical    - Words alphabetical sorting"
    echo ""
    echo "  Euler Problems:"
    echo "    euler_problem_13_sol1    - Project Euler problem 13"
    echo "    euler_problem_20_sol1    - Project Euler problem 20"
    echo "    euler_problem_22_sol1    - Project Euler problem 22"
    echo "    euler_problem_23_sol2    - Project Euler problem 23"
    echo "    euler_problem_401_sol1   - Project Euler problem 401"
    echo ""
    echo "  Games:"
    echo "    games_hangman            - Hangman game"
    echo "    games_naval_battle       - Naval Battle (Battleship) game"
    echo "    games_tic_tac_toe        - Tic-Tac-Toe game"
    echo ""
    echo "  Greedy Algorithms:"
    echo "    greedy_prim              - Prim's minimum spanning tree"
    echo ""
    echo "  Hashing:"
    echo "    hash_blake2b             - BLAKE2b cryptographic hash"
    echo ""
    echo "  Math:"
    echo "    math_prime_factorization - Prime factorization"
    echo ""
    echo "  Miscellaneous:"
    echo "    misc_mcnaughton_yamada_thompson - McNaughton-Yamada-Thompson algorithm (regex to NFA)"
    echo "    misc_poly_add            - Polynomial addition"
    echo "    misc_shunting_yard       - Shunting yard algorithm"
    echo "    misc_sudoku_solver       - Sudoku solver"
    echo ""
    echo "  Machine Learning:"
    echo "    ml_adaline_learning      - ADALINE (Adaptive Linear Neuron) learning"
    echo "    ml_k_means_clustering    - K-Means clustering algorithm"
    echo "    ml_kohonen_som_topology  - Kohonen Self-Organizing Map (topology)"
    echo "    ml_kohonen_som_trace     - Kohonen Self-Organizing Map (trace)"
    echo ""
    echo "  Numerical Methods:"
    echo "    num_durand_kerner_roots  - Durand-Kerner root finding"
    echo "    num_ode_forward_euler    - Forward Euler ODE solver"
    echo "    num_ode_midpoint_euler   - Midpoint Euler ODE solver"
    echo "    num_ode_semi_implicit_euler - Semi-implicit Euler ODE solver"
    echo "    num_realtime_stats       - Real-time statistics"
    echo ""
    echo "  Scheduling:"
    echo "    sched_non_preemptive_priority - Non-preemptive priority scheduling"
    echo ""
    echo "  Sorting:"
    echo "    sorting_bucket_sort      - Bucket sort"
    echo "    sorting_heap_sort_2      - Heap sort (variant 2)"
    echo "    sorting_merge_sort       - Merge sort"
    echo "    sorting_multikey_quick_sort - Multikey QuickSort (string sorting)"
    echo "    sorting_patience_sort    - Patience sort"
    echo ""
    echo "  Test:"
    echo "    simple_calc              - Simple calculator (add, factorial, find_max, reverse)"
    echo ""
...
# All algorithm subjects follow the pattern: subjects/<subject>/<subject>.c with no separate header
...
SUPPORTED_SUBJECTS=(
    ...
    "cipher_affine"
    # Conversions
    "conv_decimal_to_any_base"
    "conv_infix_to_postfix"
    "conv_infix_to_postfix2"
    # Dynamic Programming
    "dp_lcs"
    # Data Structures
    "ds_ascending_priority_queue"
    "ds_avl_tree"
    "ds_binary_search_tree"
    "ds_circular_doubly_linked_list"
    ...
    "ds_red_black_tree"
    "ds_segment_tree"
    "ds_singly_link_list_deletion"
    "ds_threaded_binary_trees"
    "ds_trie"
    "ds_vector"
    "ds_words_alphabetical"
    # Euler Problems
    "euler_problem_13_sol1"
    "euler_problem_20_sol1"
    "euler_problem_22_sol1"
    "euler_problem_23_sol2"
    "euler_problem_401_sol1"
    # Games
    "games_hangman"
    "games_naval_battle"
    "games_tic_tac_toe"
    ...
    # Miscellaneous
    "misc_mcnaughton_yamada_thompson"
    "misc_poly_add"
    "misc_shunting_yard"
    "misc_sudoku_solver"
    # Machine Learning
    "ml_adaline_learning"
    "ml_k_means_clustering"
    "ml_kohonen_som_topology"
    "ml_kohonen_som_trace"
    # Numerical Methods
    "num_durand_kerner_roots"
    "num_ode_forward_euler"
    "num_ode_midpoint_euler"
    "num_ode_semi_implicit_euler"
    "num_realtime_stats"
    # Scheduling
    "sched_non_preemptive_priority"
    # Sorting
    "sorting_bucket_sort"
    "sorting_heap_sort_2"
    "sorting_merge_sort"
    "sorting_multikey_quick_sort"
    "sorting_patience_sort"
    # Test
    "simple_calc"
)

# Check if subject is in the supported list
is_supported=false
for supported in "${SUPPORTED_SUBJECTS[@]}"; do
    if [ "$SUBJECT" == "$supported" ]; then
        is_supported=true
        break
    fi
done
...
if [ "$is_supported" == "false" ]; then
    echo "Error: Unsupported subject '$SUBJECT'."
    echo ""
    echo "Run '$0' without arguments to see the list of supported subjects."
    exit 1
fi

# All subjects follow the same pattern: subjects/<subject>/<subject>.c
SOURCE_FILE="subjects/$SUBJECT/$SUBJECT.c"
HEADER_FILES=""
...
# Auto-generate header file if it doesn't exist (for self-contained algorithm subjects)
if [ ! -f "$HEADER_FILES" ]; then
    echo "📝 Header file not found, auto-generating from source..."
    mkdir -p "$(dirname "$HEADER_FILES")"
    python3 utils/gen_headers_clang.py "$SOURCE_FILE" "$HEADER_FILES"
    if [ $? -eq 0 ]; then
        echo "✅ Header file generated: $HEADER_FILES"
    else
        echo "⚠️  Warning: Failed to generate header file"
    fi
fi
...
if should_perform "4"; then
    ...
    # Post-process helpers.c: inject stdout_capture_t struct if referenced but not defined
    if [ -f "$OUTPUT_PATH/helpers.c" ] && grep -q "stdout_capture_t" "$OUTPUT_PATH/helpers.c" && ! grep -q "typedef struct.*stdout_capture_t" "$OUTPUT_PATH/helpers.c"; then
        echo "🔧 Post-processing helpers.c: injecting stdout_capture_t struct and missing includes..."
        # Build the block to inject after the last #include line
        INJECT_BLOCK='#include <unistd.h>\n#include <fcntl.h>\n\ntypedef struct {\n    char *buffer;\n    size_t buffer_size;\n    int pipe_read_fd;\n    int pipe_write_fd;\n    int saved_stdout_fd;\n} stdout_capture_t;\n'
        # Find the last #include line number and inject after it
        LAST_INCLUDE_LINE=$(grep -n '^#include' "$OUTPUT_PATH/helpers.c" | tail -1 | cut -d: -f1)
        if [ -n "$LAST_INCLUDE_LINE" ]; then
            # Add unistd.h and fcntl.h if not already present
            if ! grep -q '<unistd.h>' "$OUTPUT_PATH/helpers.c"; then
                sed -i "${LAST_INCLUDE_LINE}a\\#include <unistd.h>" "$OUTPUT_PATH/helpers.c"
                LAST_INCLUDE_LINE=$((LAST_INCLUDE_LINE + 1))
            fi
            if ! grep -q '<fcntl.h>' "$OUTPUT_PATH/helpers.c"; then
                sed -i "${LAST_INCLUDE_LINE}a\\#include <fcntl.h>" "$OUTPUT_PATH/helpers.c"
                LAST_INCLUDE_LINE=$((LAST_INCLUDE_LINE + 1))
            fi
            # Inject the struct typedef after includes
            sed -i "${LAST_INCLUDE_LINE}a\\\\\ntypedef struct {\\\n    char *buffer;\\\n    size_t buffer_size;\\\n    int pipe_read_fd;\\\n    int pipe_write_fd;\\\n    int saved_stdout_fd;\\\n} stdout_capture_t;" "$OUTPUT_PATH/helpers.c"
            echo "✅ stdout_capture_t struct injected into helpers.c"
        fi
    fi
fi
...
# Generate metrics.txt from validation_metrics.json
# This runs after step 4 (validation) or with "info" or "all"
METRICS_JSON="$OUTPUT_PATH/validation_metrics.json"
METRICS_TXT="$OUTPUT_PATH/metrics.txt"

if [ -f "$METRICS_JSON" ] && (should_perform "4" || should_perform "info"); then
    echo ""
    echo "┌──────────────────────────────────────────────────────────────────────────┐"
...
lines.append('=== Validation Metrics for $SUBJECT ===')
lines.append('')
lines.append(f'1. Total tests generated (before validation):  {m[\"total_tests_before_validation\"]}')
lines.append(f'2. Total tests in final unit_test.c:            {m[\"total_tests_in_final_unit_test\"]}')
lines.append(f'3. Tests that required repair (any iteration):  {m[\"tests_needed_repair\"]}')
lines.append(f'4. Tests repaired successfully:                 {m[\"tests_repaired_successfully\"]}')
lines.append('')
lines.append('5. Tests fixed per iteration:')
fix_iter = m.get('fixed_after_iteration', {})
max_iter = m.get('max_iterations_configured', 3)
for i in range(1, max_iter + 1):
    count = fix_iter.get(str(i), 0)
    lines.append(f'     Iteration {i}: {count}')
lines.append('')
lines.append('6. Repaired failure types:')
ft = m.get('repaired_failure_types', {})
for ftype in ['compilation', 'assertion', 'crash', 'memory', 'timeout']:
    count = ft.get(ftype, 0)
    lines.append(f'     {ftype:.<40s} {count}')
lines.append('')
lines.append('--- Additional ---')
lines.append(f'   Tests passed initially (no repair):          {m.get(\"tests_passed_initially\", \"N/A\")}')
lines.append(f'   Tests dropped (validation failed):           {m.get(\"tests_dropped\", \"N/A\")}')
...
" 2>/dev/null
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "│  ⚠️  Could not parse validation_metrics.json"
    fi
    echo "└──────────────────────────────────────────────────────────────────────────┘"
fi
